...
>
    ...
    <xsl:template
            match="indexterm[@class='startofrange'][not(*/@sortas)] | indexterm[@class='startofrange'][parent::emphasis][not(*/@sortas)]">
        <xsl:choose>
            <xsl:when test="$strip-indexterms = 'true'"/>
            <xsl:otherwise>
                <xsl:text>(((</xsl:text>
                <xsl:apply-templates/>
                <xsl:text>, id="</xsl:text><xsl:value-of select="@id"/><xsl:text>", range="startofrange")))</xsl:text>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>

    <xsl:template
            match="indexterm[*/@sortas][not(@class='startofrange')] | indexterm[*/@sortas][parent::emphasis][not(@class='startofrange')]">
        <xsl:choose>
            <xsl:when test="$strip-indexterms = 'true'"/>
            ...
            <xsl:otherwise>
                <xsl:text>(((</xsl:text>
                <xsl:apply-templates/>
                <xsl:text>, sortas="</xsl:text><xsl:value-of select="primary/@sortas"/><xsl:text>")))</xsl:text>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>
...
== Glossary
            ...
            <xsl:otherwise>
                <xsl:call-template name="process-id"/>==
                <xsl:value-of select="title"/><xsl:value-of select="util:carriage-returns(2)"/>[glossary]<xsl:value-of
                    select="util:carriage-returns(1)"/>
                <xsl:apply-templates select="*[not(self::title)]"/>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>
    ...
    <xsl:template match="glossseealso">
        <xsl:choose>
            <xsl:when test="preceding-sibling::para">
                <xsl:text>+&#10;See Also </xsl:text><xsl:value-of select="id(@otherterm)/glossterm"/>
                <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:text>See Also </xsl:text><xsl:value-of select="id(@otherterm)/glossterm"/><xsl:value-of
                    select="util:carriage-returns(2)"/>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>
    ...
    <xsl:template match="para|simpara">
        ...
        <xsl:if test="ancestor::listitem and preceding-sibling::element()">
            <xsl:choose>
                <xsl:when
                        test="not(ancestor::warning|ancestor::note|ancestor::caution|ancestor::tip|ancestor::important) and not(ancestor::sidebar)">
    ...
    <xsl:template match="term"><xsl:apply-templates select="node()"/>::
    </xsl:template>
    ...
    <xsl:template
            match="emphasis[@role='bold'][not(ancestor::programlisting)]|emphasis[@role='strong'][not(ancestor::programlisting)]">
        **
        <xsl:if test="(contains(., '~') or contains(., '_') or contains(., '^')) and not(descendant::*)">$$</xsl:if>
        <xsl:apply-templates/>
        <xsl:if test="(contains(., '~') or contains(., '_') or contains(., '^')) and not(descendant::*)">$$</xsl:if>**
    </xsl:template>
    ...
    <xsl:template match="filename">__
        <xsl:if test="contains(., '~') or contains(., '_')">$$</xsl:if>
        <xsl:apply-templates/>
        <xsl:if test="contains(., '~') or contains(., '_')">$$</xsl:if>__
    </xsl:template>
    ...
    <xsl:template match="emphasis[not(ancestor::programlisting)]">__
        <xsl:if test="(contains(., '~') or contains(., '_') or contains(., '^')) and not(descendant::*)">$$</xsl:if>
        <xsl:apply-templates/>
        <xsl:if test="(contains(., '~') or contains(., '_') or contains(., '^')) and not(descendant::*)">$$</xsl:if>__
    </xsl:template>
    ...
    <xsl:template match="literal | code">++
        <xsl:if test='(contains(., "+") or contains(., "&apos;") or contains(., "_") or contains(., "^") or contains(., "~")) and not(descendant::*)'>
            $$
        </xsl:if>
        <xsl:apply-templates/>
        <xsl:if test='(contains(., "+") or contains(., "&apos;") or contains(., "_") or contains(., "^") or contains(., "~")) and not(descendant::*)'>
    ...
    <xsl:template match="literal/text()">
        <xsl:value-of
                select="replace(replace(replace(., '\n\s+', ' ', 'm'), 'C\+\+', '\$\$C++\$\$', 'm'), '([\*\^~])', '\\$1', 'm')"></xsl:value-of>
    </xsl:template>

    <xsl:template match="userinput">**`<xsl:apply-templates/>`**
    </xsl:template>
    ...
    <xsl:template match="replaceable">
        <xsl:choose>
            <xsl:when test="parent::literal">__<xsl:apply-templates/>__
            </xsl:when>
            <xsl:otherwise>__++<xsl:apply-templates/>++__
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>
    ...
    <xsl:template match="superscript">^<xsl:apply-templates/>^
    </xsl:template>
    ...
    <xsl:template match="subscript">~<xsl:apply-templates/>~
    </xsl:template>
    ...
    <xsl:template match="ulink">link:$$<xsl:value-of select="@url"/>$$[<xsl:apply-templates/>]
    </xsl:template>

    <xsl:template match="email">
        <xsl:text>pass:[</xsl:text>
        <xsl:element name="a">
            <xsl:attribute name="href">mailto:<xsl:value-of select="normalize-space(.)"/>
            </xsl:attribute>
            <xsl:element name="em">
                <xsl:value-of select="normalize-space(.)"/>
            </xsl:element>
    ...
    <xsl:template match="xref">&#xE801;&#xE801;<xsl:value-of select="@linkend"/>&#xE802;&#xE802;
    </xsl:template>

    <xsl:template match="link">&#xE801;&#xE801;<xsl:value-of select="@linkend"/>,<xsl:value-of select="."/>&#xE802;&#xE802;
    </xsl:template>
                        ...
                        <xsl:element name="dl"><xsl:text>
                            ...
                            <xsl:for-each select="varlistentry">
                                <xsl:for-each select="term">
                                    <xsl:element name="dt">
                                        <xsl:copy-of select="self::term"/>
                                    </xsl:element>
                                    <xsl:text>
</xsl:text>
                                </xsl:for-each>
                                <xsl:element name="dd">
                                    <xsl:copy-of select="listitem"/>
                                </xsl:element>
                                <xsl:text>
</xsl:text>
                            </xsl:for-each>
...
++++</xsl:text>
    ...
    <xsl:template match="inlinemediaobject">image:<xsl:value-of select="imageobject[@role='web']/imagedata/@fileref"/>[]
    </xsl:template>
    ...
    <!-- <co> element -->
    <xsl:template match="co" mode="code"><xsl:variable name="curr" select="@id"/>&#xE801;<xsl:value-of
        ...
        <xsl:choose>
            ...
            <xsl:when test="callout/id(@arearefs)/ancestor::example">
                <xsl:choose>
                    ...
                    also output calloutlist as Docbook passthrough.-->
                    <xsl:when
                            test="callout/id(@arearefs)/parent::*/parent::example/parent::node() != self::calloutlist/parent::node()">
                        ++++++++++++++++++++++++++++++++++++++
                        <xsl:copy-of select="."/>
                        ++++++++++++++++++++++++++++++++++++++
