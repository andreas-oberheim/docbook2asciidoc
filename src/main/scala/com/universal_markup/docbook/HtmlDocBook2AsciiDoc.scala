...
<xsl:text>+++</xsl:text>
      ...
      <xsl:template match="phrase/text()" mode="d2a">
        <xsl:text/>
      ...
      <xsl:template match="sect1" mode="d2a">
        ...
        <xsl:apply-templates select="title" mode="d2a"/>
        <xsl:value-of select="util:carriage-returns(2)"/>
        <xsl:if test="sect1info and not(sect1info//author[2])">__By
          <xsl:value-of select="normalize-space(sect1info//firstname)"/> <xsl:text></xsl:text> <xsl:value-of select="normalize-space(sect1info//surname)"/>
          __
          <xsl:value-of select="util:carriage-returns(2)"/>
        </xsl:if>
	...
	proper AsciiDoc attribute processing -->
      ...
      <xsl:template match="replaceable" mode="d2a">
        <xsl:choose>
          <xsl:when test="parent::literal or parent::code or parent::command or parent::option or parent::computeroutput or parent::varname or parent::classname or parent::code or parent::envar or parent::constant or parent::function or parent::methodname or parent::package or parent::paramater or parent::property">__
            <xsl:apply-templates mode="d2a"/>
            __</xsl:when>
          <xsl:otherwise>__++
            <xsl:apply-templates mode="d2a"/>
      ...
      <xsl:template match="phrase" mode="d2a">
        <xsl:apply-templates mode="d2a"/>
      </xsl:template>

      <xsl:template match="userinput" mode="d2a">**`
        <xsl:apply-templates mode="d2a"/>
        `**</xsl:template>

      <xsl:template match="ulink" mode="d2a">link:$$
        <xsl:value-of select="@url"/>
        $$[
        <xsl:apply-templates mode="d2a"/>
        ]</xsl:template>

      <xsl:template match="email" mode="d2a">
      ...
      <xsl:template match="xref" mode="d2a">
        &#xE801; &#xE801; <xsl:value-of select="@linkend"/> &#xE802; &#xE802;
      </xsl:template>

      <xsl:template match="link" mode="d2a">
        &#xE801; &#xE801; <xsl:value-of select="@linkend"/>
        ,
        <xsl:value-of select="."/> &#xE802; &#xE802;
      </xsl:template>
      ...
      <xsl:template match="inlinemediaobject" mode="d2a">image:
...
also output calloutlist as passthrough.-->
              ...
              <xsl:otherwise>
                <xsl:for-each select="callout">
                  &#xE801; <xsl:value-of select="position()"/> &#xE802; <xsl:apply-templates mode="d2a"/>
                </xsl:for-each>
