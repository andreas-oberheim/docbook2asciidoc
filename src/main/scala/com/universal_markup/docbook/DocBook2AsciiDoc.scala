  ...
  <xsl:output-character character="}" string="&amp;#x7d;"/> -->
      ...
      <xsl:template match="indexterm[@class='startofrange'][not(*/@sortas)] | indexterm[@class='startofrange'][parent::emphasis][not(*/@sortas)]">
        <xsl:choose>
          <xsl:when test="$strip-indexterms = 'true'"/>
          <xsl:otherwise>
            <xsl:text>(((</xsl:text> <xsl:apply-templates/> <xsl:text>, id="</xsl:text> <xsl:value-of select="@id"/> <xsl:text>", range="startofrange")))</xsl:text>
          </xsl:otherwise>
        </xsl:choose>
      </xsl:template>
      ...
      <xsl:template match="indexterm[*/@sortas][not(@class='startofrange')] | indexterm[*/@sortas][parent::emphasis][not(@class='startofrange')]">
        <xsl:choose>
          ...
          <xsl:when test="$strip-indexterms = 'false' and secondary">
            <xsl:text>pass:[</xsl:text> <xsl:copy-of select="."/> <xsl:text>]</xsl:text>
          </xsl:when>
          <!-- When only primary term exists, output as asciidoc -->
          <xsl:otherwise>
            <xsl:text>(((</xsl:text> <xsl:apply-templates/> <xsl:text>, sortas="</xsl:text> <xsl:value-of select="primary/@sortas"/> <xsl:text>")))</xsl:text>
          </xsl:otherwise>
        </xsl:choose>
      </xsl:template>
      ...
      <xsl:template match="indexterm/primary | indexterm/primary[parent::emphasis]">
        <xsl:choose>
          <xsl:when test="$strip-indexterms = 'true'"/>
          <xsl:otherwise>
            <xsl:text>"</xsl:text> <xsl:if test="contains(., '+') or contains(., '_') or contains(., '#')">$$</xsl:if> <xsl:apply-templates/> <xsl:if test="contains(., '+') or contains(., '_') or contains(., '#')">$$</xsl:if> <xsl:text>"</xsl:text>
          </xsl:otherwise>
        </xsl:choose>
      </xsl:template>
      ...
      <xsl:template match="indexterm/see | indexterm/see[parent::emphasis]">
        <xsl:choose>
          <xsl:when test="$strip-indexterms = 'true'"/>
          <xsl:otherwise>
            <xsl:text>, see="</xsl:text> <xsl:if test="contains(., '+') or contains(., '_') or contains(., '#')">$$</xsl:if> <xsl:apply-templates/> <xsl:if test="contains(., '+') or contains(., '_') or contains(., '#')">$$</xsl:if> <xsl:text>"</xsl:text>
          </xsl:otherwise>
        </xsl:choose>
      </xsl:template>
      <xsl:template match="indexterm/seealso | indexterm/seealso[parent::emphasis]">
        <xsl:choose>
          <xsl:when test="$strip-indexterms = 'true'"/>
          <xsl:otherwise>
            <xsl:text>, seealso="</xsl:text> <xsl:if test="contains(., '+') or contains(., '_') or contains(., '#')">$$</xsl:if> <xsl:apply-templates/> <xsl:if test="contains(., '+') or contains(., '_') or contains(., '#')">$$</xsl:if> <xsl:text>"</xsl:text>
          </xsl:otherwise>
        </xsl:choose>
      </xsl:template>
      <!-- END INDEX HANDLING -->
    ...
    currently retain any @id attrbutes for glossentry elements. -->
      <xsl:template match="glossseealso">
        <xsl:choose>
          ...
          <xsl:otherwise>
            <xsl:text>See Also</xsl:text> <xsl:value-of select="id(@otherterm)/glossterm"/>
            <xsl:value-of select="util:carriage-returns(2)"/>
          </xsl:otherwise>
        </xsl:choose>
      </xsl:template>
	...
	proper AsciiDoc attribute processing -->
      ...
      <xsl:template match="emphasis[@role='bold'][not(ancestor::programlisting)]|emphasis[@role='strong'][not(ancestor::programlisting)]">**
        <xsl:if test="(contains(., '~') or contains(., '_') or contains(., '^')) and not(descendant::*)">$$</xsl:if> <xsl:apply-templates/> <xsl:if test="(contains(., '~') or contains(., '_') or contains(., '^')) and not(descendant::*)">$$</xsl:if>
        **</xsl:template>
      ...
      <xsl:template match="filename">__
        <xsl:if test="contains(., '~') or contains(., '_')">$$</xsl:if> <xsl:apply-templates/> <xsl:if test="contains(., '~') or contains(., '_')">$$</xsl:if>
        __</xsl:template>
      ...
      <xsl:template match="literal | code">++
        <xsl:if test='(contains(., "+") or contains(., "&apos;") or contains(., "_") or contains(., "^") or contains(., "~")) and not(descendant::*)'>$$</xsl:if> <xsl:apply-templates/> <xsl:if test='(contains(., "+") or contains(., "&apos;") or contains(., "_") or contains(., "^") or contains(., "~")) and not(descendant::*)'>$$</xsl:if>
        ++</xsl:template>
      ...
      <xsl:template match="replaceable">
        <xsl:choose>
          <xsl:when test="parent::literal">__
            <xsl:apply-templates/>
            __</xsl:when>
          <xsl:otherwise>__++
            <xsl:apply-templates/>
      ...
      <xsl:template match="email">
        <xsl:text>pass:[</xsl:text> <xsl:element name="a">
        <xsl:attribute name="href">mailto:
          <xsl:value-of select="normalize-space(.)"/>
        </xsl:attribute> <xsl:element name="em">
          <xsl:value-of select="normalize-space(.)"/>
        </xsl:element>
      ...
      <xsl:template match="inlinemediaobject">image:
      ...
      <xsl:template match="co" mode="code">
        <xsl:variable name="curr" select="@id"/> &#xE801; <xsl:value-of select="count(//calloutlist/callout[@arearefs=$curr]/preceding-sibling::callout)+1"/> &#xE802;
      </xsl:template>
